...
class ExcelLiteTransferController extends BaseController
{
    ...
    private array $relations = [
        'department_code' => ['field' => 'department_id', 'table' => 'departments', 'tenant' => true, 'site' => true],
        'warehouse_code' => ['field' => 'warehouse_id', 'table' => 'warehouses', 'tenant' => true, 'site' => true],
        'location_code' => ['field' => 'location_id', 'table' => 'locations', 'tenant' => true, 'site' => true],
    ...
    public function __construct()
    {
        $this->resources = [
            ...
            'supplier-terms' => ['title' => 'Supplier Terms', 'table' => 'supplier_terms', 'tenant' => true, 'site' => true, 'fields' => ['supplier', 'supplier_name', 'terms_code', 'terms_name', 'terms_days', 'promo_code', 'is_active'], 'key' => 'terms_code', 'permission' => 'purchase.supplier'],
    ...
    private function normalizeBeforeSave(array $config, array $data, int $rowNumber): array
    {
        ...
        elseif ($config['table'] === 'item_locations') $data['item_code'] = $this->lookupCodeById('items', (int) ($data['item_id'] ?? 0));
        elseif ($config['table'] === 'batch_masters') $data['item_code'] = $this->lookupCodeById('items', (int) ($data['item_id'] ?? 0));
        ...
        elseif ($config['table'] === 'customer_terms' && ! empty($data['customer']) && empty($data['customer_name'])) $data['customer_name'] = $this->lookupNameByCode('customers', (string) $data['customer'], 'customern', true, true, $data['company_id'] ?? null, $data['site_id'] ?? null);
        elseif ($config['table'] === 'supplier_terms' && ! empty($data['supplier']) && empty($data['supplier_name'])) $data['supplier_name'] = $this->lookupNameByCode('suppliers', (string) $data['supplier'], 'supplierna', true, true, $data['company_id'] ?? null, $data['site_id'] ?? null);
        return $data;
    }
    ...
    private function lookupNameByCode(string $table, string $code, string $nameColumn, bool $tenant, bool $site, ?int $companyId, ?int $siteId): ?string
    {
        if ($code === '') return null;
        $db = Database::connect();
        $builder = $db->table($table)->where($this->codeColumn($table), $code);
    ...
    private function sampleRow(array $config): array { return array_map(function (string $header) use ($config): string { return match ($header) { 'company_code' => 'PENA', 'site_code' => 'HO', 'code' => 'EXAMPLE', 'name' => 'Example ' . $config['title'], 'description' => 'Example description', 'department_code' => 'GEN', 'warehouse_code', 'stockwhs', 'shipwhs' => 'MAIN', 'location_code' => 'A01', 'country_code' => 'IDN', 'province_code' => 'DKI', 'city_code' => 'JKT', 'postal_code' => '12190', 'from_uom_code', 'to_uom_code', 'stockuom', 'purchaseuom', 'sellinguom' => 'PCS', 'item_code_ref', 'item_code' => 'ITEM001', 'vat_code', 'vat' => 'VAT11', 'customer' => 'CUST-999', 'customer_name', 'customern' => 'Example Customer', 'supplier' => 'SUP-999', 'supplier_name', 'supplierna' => 'Example Supplier', 'terms_code', 'terms' => 'NET30', 'promo_code' => 'PROMO-001', 'line_no' => '1', 'email' => 'demo@example.com', 'is_default', 'is_active', 'active' => '1', 'rate', 'item_price', 'purchasep', 'sellingprice', 'multiplier', 'divider', 'terms_days', 'min_qty', 'max_qty', 'reorder_qty' => '0', default => '', }; }, $this->excelHeaders($config)); }
