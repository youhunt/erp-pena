...
class OrderImportController extends BaseController
{
    ...
    private const SALES_REQUIRED_HEADERS = [
        'so_no', 'so_line', 'so_date', 'site_code', 'customer_code',
        'item_code', 'qty', 'uom_code', 'unit_price',
    ];
    ...
    private const PURCHASE_HEADERS = [
        ...
        'header_discount_percent', 'header_discount_amount', 'freight_amount', 'other_amount',
        'special_charge_amount', 'vat_amount', 'wht_amount',
        'item_code', 'item_name', 'description', 'qty', 'uom_code', 'unit_price',
        'line_discount_percent', 'line_discount_amount', 'line_vat_amount', 'line_wht_amount',
    ];

    private const PURCHASE_REQUIRED_HEADERS = [
        'po_no', 'po_line', 'po_date', 'site_code', 'supplier_code',
        'item_code', 'qty', 'uom_code', 'unit_price',
    ];
    ...
    private function buildPreview(string $type, array $records, TenantContext $tenant, string $filename): array
    {
        ...
        foreach ($groups as $groupKey => $lines) {
            $first = $lines[0];
            $documentNo = trim((string) ($first[$config['documentField']] ?? ''));
        ...
        $validDocuments = count($validGroupKeys);
        $message = $errors === []
        ...
        foreach ($lines as $line) {
            foreach ($checks as $field => $label) {
                ...
                if ($lineValue !== '' && $firstValue !== '' && $firstValue !== $lineValue) {
                    return 'Rows with the same document number and site must use the same ' . $label . '.';
                }
            }
        }
    ...
    private function createDocuments(string $type, array $records, TenantContext $tenant): array
    {
        ...
        foreach ($groups as $lines) {
            ...
            if ($type === 'purchase') {
                $header += $this->purchaseHeaderAmounts($lines, $first);
            }
    ...
    private function purchaseHeaderAmounts(array $lines, array $first): array
    {
        $discountPercent = $this->firstNumber($lines, ['header_discount_percent', 'discount_header_percent']);
        ...
        return [
            ...
            'discount_amount' => $this->sumNumbers($lines, ['header_discount_amount', 'discount_header_amount']),
            'freight_amount' => $this->sumNumbers($lines, ['freight_amount']),
            'other_amount' => $this->sumNumbers($lines, ['other_amount']),
            'special_charge_amount' => $this->sumNumbers($lines, ['special_charge_amount']),
            'vat_amount' => $this->sumNumbers($lines, ['vat_amount']),
            'wht_amount' => $this->sumNumbers($lines, ['wht_amount']),
        ];
    }
    ...
    private function linePayload(array $line, string $type, ?int $siteId = null): array
    {
        ...
        if ($type === 'sales') {
            ...
            $discountPercent = $this->number($line['line_discount_percent'] ?? $line['discount_percent'] ?? 0);
            $discountAmount = $this->number($line['line_discount_amount'] ?? $line['discount_amount'] ?? 0);
            if ($discountPercent > 0) {
                $discountAmount += round($gross * $discountPercent / 100, 2);
            }
            $vatAmount = $this->number($line['line_vat_amount'] ?? 0);
            $whtAmount = $this->number($line['line_wht_amount'] ?? 0);
            $payload += [
                'discount_percent' => $discountPercent,
                'discount_amount' => $discountAmount,
                'vat_amount' => $vatAmount,
                'wht_amount' => $whtAmount,
                'tax_amount' => $vatAmount - $whtAmount,
            ];
        }
    ...
    private function rowsToRecords(array $rows, array $requiredHeaders): array
    {
        ...
        foreach ($requiredHeaders as $header) {
            if (! $this->hasHeader($header, $headers)) {
        ...
        foreach (array_slice($rows, 1) as $index => $row) {
            ...
            $records[] = $this->applyHeaderAliases($record);
        }
    ...
    private function applyHeaderAliases(array $record): array
    {
        foreach ($this->headerAliases() as $canonical => $aliases) {
            if (trim((string) ($record[$canonical] ?? '')) !== '') {
                continue;
            }
    ...
    private function headerAliases(): array
    {
        return [
            'site_code' => ['site'],
            'header_discount_percent' => ['discount_header_percent'],
            'header_discount_amount' => ['discount_header_amount'],
            'line_discount_percent' => ['discount_percent'],
            'line_discount_amount' => ['discount_amount'],
            'line_vat_amount' => ['vat_line_amount'],
            'line_wht_amount' => ['wht_line_amount'],
        ];
    }

    private function documentGroupKey(array $row, array $config): string
    {
        return strtoupper(trim((string) ($row[$config['documentField']] ?? ''))) . '|' . strtoupper($this->siteCode($row));
    }
    ...
    private function assertNotDuplicate(string $table, string $documentField, string $documentNo, int $companyId, ?int $siteId = null): void
    {
        ...
        if ($builder->countAllResults() > 0) {
            throw new RuntimeException(strtoupper(str_replace('_no', '', $documentField)) . ' number already exists for this site: ' . $documentNo);
        }
    }
    ...
    private function lookupPartner(string $type, string $code, ?int $companyId = null, ?int $siteId = null): ?array
    {
        ...
        if ($db->fieldExists($legacyCode, $table)) {
            $builder->where($legacyCode, $code)->orWhere('code', $code);
        } else {
    ...
    private function lookupItem(string $code, ?int $siteId = null): ?array
    {
        ...
        $db = Database::connect();
        $builder = $db->table('items')->groupStart();
        if ($db->fieldExists('item_code', 'items')) {
            $builder->where('item_code', $code)->orWhere('code', $code);
        } else {
            $builder->where('code', $code);
        }
        $builder->groupEnd();
    ...
    {
        $db = Database::connect();
        if ($db->fieldExists('company_id', $table) && $companyId !== null) {
            $builder->where('company_id', $companyId);
        }
        if ($db->fieldExists('site_id', $table) && $siteId !== null) {
            $builder->where('site_id', $siteId);
        }
        if ($db->fieldExists('deleted_at', $table)) {
            $builder->where('deleted_at', null);
    ...
    private function resolveSiteContext(mixed $siteCode, TenantContext $tenant, int $rowNumber): array
    {
        ...
        $db = Database::connect();
        $builder = $db->table('sites')->where('company_id', $companyId)->groupStart();
        $hasCode = $db->fieldExists('code', 'sites');
        $hasLegacySite = $db->fieldExists('site', 'sites');
        if ($hasCode) {
            $builder->where('code', $siteCode);
        }
        if ($hasLegacySite) {
            $hasCode ? $builder->orWhere('site', $siteCode) : $builder->where('site', $siteCode);
        }
        if (! $hasCode && ! $hasLegacySite) {
            throw new RuntimeException('Sites table has no code/site column.');
        }
        $builder->groupEnd();
        ...
        if ($site === null) {
            throw new RuntimeException('Site not found on Excel row ' . $rowNumber . ': ' . $siteCode);
        }

        $siteId = (int) $site['id'];
        if (! $tenant->userCanAccessSite((int) auth()->id(), $companyId, $siteId)) {
            throw new RuntimeException('You do not have access to site ' . $siteCode . ' on Excel row ' . $rowNumber . '.');
        }
    ...
    private function normalizeOptionalDate(mixed $value, int $rowNumber, string $label): ?string
    {
        ...
        $timestamp = strtotime($value);
        if ($timestamp === false) {
            throw new RuntimeException('Invalid ' . $label . ' on Excel row ' . $rowNumber . '. Use YYYY-MM-DD.');
        }
        return date('Y-m-d', $timestamp);
    }
    ...
    private function sumNumbers(array $lines, array $fields): float
    {
        $sum = 0.0;
        foreach ($lines as $line) {
            foreach ($fields as $field) {
                if (isset($line[$field]) && trim((string) $line[$field]) !== '') {
                    $sum += $this->number($line[$field]);
                    break;
                }
            }
        }
    ...
    private function firstNumber(array $lines, array $fields): float
    {
        foreach ($lines as $line) {
            foreach ($fields as $field) {
                if (isset($line[$field]) && trim((string) $line[$field]) !== '') {
                    return $this->number($line[$field]);
                }
            }
        }
        return 0.0;
    ...
    private function config(string $type): array
    {
        if ($type === 'sales') {
            return [
                ...
                'requiredHeaders' => self::SALES_REQUIRED_HEADERS,
                'sampleRows' => [
        ...
        return [
            ...
            'requiredHeaders' => self::PURCHASE_REQUIRED_HEADERS,
            'sampleRows' => [
                ['PO-IMPORT-001', '1', date('Y-m-d'), 'HO', date('Y-m-d', strtotime('+3 days')), date('Y-m-d', strtotime('+5 days')), 'SUP001', 'PT Contoh Supplier', 'NET30', 'IDR', 'Contoh import PO', 'Header remarks', '0', '0', '15000', '0', '5000', '11000', '0', 'ITEM-0001', 'Kertas A4 80gsm 001', 'Description line 1', '20', 'PCS', '20000', '2.5', '0', '0', '0'],
                ['PO-IMPORT-001', '2', date('Y-m-d'), 'HO', date('Y-m-d', strtotime('+3 days')), date('Y-m-d', strtotime('+5 days')), 'SUP001', 'PT Contoh Supplier', 'NET30', 'IDR', 'Contoh import PO', 'Header remarks', '0', '0', '25000', '0', '0', '0', '0', 'ITEM-0002', 'Pulpen Hitam 002', 'Description line 2', '12', 'PCS', '4000', '0', '1000', '0', '0'],
            ],
