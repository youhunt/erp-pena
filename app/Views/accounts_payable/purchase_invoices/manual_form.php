...
<form method="post" action="<?= site_url('ap/manual-invoices') ?>">
    <?= csrf_field() ?>
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
                <div>
                    <h4 class="card-title mb-1">Manual A/P Invoice</h4>
                    <p class="text-muted mb-0">Post supplier invoice directly into A/P payable and GL.</p>
                </div>
                <a href="<?= site_url('ap/purchase-invoices') ?>" class="btn btn-light">Back</a>
            </div>

            <div class="row g-3">
                <div class="col-md-3">
                    <label class="form-label">Invoice No</label>
                    <input type="text" name="invoice_no" class="form-control" required value="<?= esc(old('invoice_no', 'MPI-' . date('Ymd-His'))) ?>">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="supplier_id" class="form-select" required>
                        ...
                        <?php foreach ($suppliers as $supplier): ?>
                            <?php $supplierLabel = ($supplier['code'] ?? $supplier['supplier'] ?? '-') . ' - ' . ($supplier['name'] ?? $supplier['supplierna'] ?? '-'); ?>
                            <option value="<?= esc($supplier['id']) ?>" <?= old('supplier_id') == $supplier['id'] ? 'selected' : '' ?>><?= esc($supplierLabel) ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="col-md-6">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="d-flex gap-2 mt-4">
                <button type="submit" class="btn btn-primary" onclick="return confirm('Post this manual A/P invoice?')"><i class="bx bx-receipt me-1"></i> Post Invoice</button>
                <a href="<?= site_url('ap/purchase-invoices') ?>" class="btn btn-light">Cancel</a>
            </div>
        </div>
    </div>
</form>
        ...
        <td>
            <select class="form-select form-select-sm item-select" name="line_item_id[]">
                ...
                <?php foreach ($items as $item): ?>
                    ...
                    <option value="<?= esc($item['id']) ?>" data-code="<?= esc($itemCode) ?>" data-name="<?= esc($itemName) ?>" data-uom="<?= esc($item['purchaseuom'] ?? $item['stockuom'] ?? 'PCS') ?>" data-price="<?= esc($item['purchasep'] ?? $item['item_price'] ?? 0) ?>">
