        ...
        <div class="table-responsive">
            <table class="table table-nowrap align-middle mb-0">
                <thead class="table-light">
                    <tr><th>Receipt No</th><th>Date</th><th>Invoice</th><th>Customer</th><th>Method</th><th>Status</th><th class="text-end">Amount</th><th></th></tr>
                </thead>
                <tbody>
                <?php foreach ($receipts as $receipt): ?>
                    <tr>
                        ...
                        <td><span class="badge <?= ($receipt['status'] ?? 'posted') === 'cancelled' ? 'bg-danger' : 'bg-success' ?>"><?= esc($receipt['status'] ?? 'posted') ?></span></td>
                ...
                <?php endforeach ?>
                <?php if ($receipts === []): ?>
                    <tr><td colspan="8" class="text-center text-muted py-4">No A/R receipt posted yet.</td></tr>
                <?php endif ?>
                </tbody>
            </table>
        </div>
