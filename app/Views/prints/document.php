...
<body>
    ...
    <div class="page">
        <div class="header">
            <div>
                <div class="company-name"><?= esc($company['name'] ?? $company['legal_name'] ?? $h['company'] ?? 'LENTERRA ERP') ?></div>
        ...
        <div class="grid">
            <div class="box">
                ...
                <table class="meta">
                    <tr>
                        <td>No</td>
                        <td class="fw"><?= esc($docNo) ?></td>
                    </tr>
                    <tr>
                        <td><?= esc($cfg['date_label'] ?? 'Date') ?></td>
                        <td><?= esc($docDate ?: '-') ?></td>
                    </tr>
                    <?php if (! empty($h['delivery_date'])): ?><tr>
                            <td>Delivery Date</td>
                            <td><?= esc($h['delivery_date']) ?></td>
                        </tr><?php endif ?>
                    <?php if (! empty($h['arrive_date'])): ?><tr>
                            <td>Arrive Date</td>
                            <td><?= esc($h['arrive_date']) ?></td>
                        </tr><?php endif ?>
                    <?php if ($sourceLabel !== '' && $sourceField !== ''): ?><tr>
                            <td><?= esc($sourceLabel) ?></td>
                            <td><?= esc($h[$sourceField] ?? '-') ?></td>
                        </tr><?php endif ?>
                    <tr>
                        <td>Currency</td>
                        <td><?= esc($h['currency_code'] ?? 'IDR') ?></td>
                    </tr>
                </table>
            </div>
            <div class="box">
                ...
                <table class="meta">
                    <tr>
                        <td>Code</td>
                        <td class="fw"><?= esc($partnerCode ?: '-') ?></td>
                    </tr>
                    <tr>
                        <td>Name</td>
                        <td><?= esc($partnerName ?: '-') ?></td>
                    </tr>
                    <tr>
                        <td>Terms</td>
                        <td><?= esc($h['terms_code'] ?? '-') ?></td>
                    </tr>
                </table>
            </div>
        </div>
        <table class="lines">
            <thead>
                <tr>
                    ...
                    <th class="text-end" style="width:86px;"><?= esc($cfg['qty_label'] ?? 'Qty') ?></th>
                    <th style="width:60px;">UoM</th>
                    <?php if ($showUnitPrice): ?><th class="text-end" style="width:90px;"><?= esc($cfg['price_label'] ?? 'Price') ?></th><?php endif ?>
                    <?php if ($showLineCommercial): ?><th class="text-end" style="width:82px;">Disc</th>
                        <th class="text-end" style="width:82px;">Tax</th><?php endif ?>
                    <?php if ($showAmounts): ?><th class="text-end" style="width:98px;">Amount</th><?php endif ?>
                </tr>
            </thead>
            <tbody>
                <?php foreach (($lines ?? []) as $line): ?>
                    <?php $lineNo = $line['line_no'] ?? $line['po_line'] ?? $line['so_line'] ?? '';
                    $lineQty = $line[$cfg['qty'] ?? 'qty'] ?? $line['qty'] ?? 0;
                    $linePrice = $line[$cfg['price'] ?? 'unit_price'] ?? $line['unit_price'] ?? $line['unit_cost'] ?? 0;
                    $lineAmount = array_key_exists('line_total', $line) ? (float) $line['line_total'] : ((float) $lineQty * (float) $linePrice); ?>
                    <tr>
                        <td class="text-center"><?= esc($lineNo) ?></td>
                        <td><?= esc($line['item_code'] ?? '-') ?></td>
                        <td>
                            <div class="fw"><?= esc($line['item_name'] ?? '-') ?></div><?php if (! empty($line['description'])): ?><div class="muted"><?= esc($line['description']) ?></div><?php endif ?><?php if (! empty($line['batch_no'])): ?><div class="muted">Batch: <?= esc($line['batch_no']) ?></div><?php endif ?>
                        </td>
                        ...
                        <?php if ($showLineCommercial): ?><td class="text-end"><?= esc($money($line['discount_amount'] ?? 0)) ?></td>
                            <td class="text-end"><?= esc($money($line['vat_amount'] ?? $line['tax_amount'] ?? 0)) ?></td><?php endif ?>
                        <?php if ($showAmounts): ?><td class="text-end fw"><?= esc($money($lineAmount)) ?></td><?php endif ?>
                    </tr>
                <?php endforeach ?>
                <?php if (($lines ?? []) === []): ?><tr>
                        <td colspan="<?= (int) $lineColspan ?>" class="text-center muted">No line data.</td>
                    </tr><?php endif ?>
            </tbody>
        </table>
        <div class="document-footer">
            <div><?php if (! empty($h['notes']) || ! empty($h['remarks'])): ?><div class="box">
                        <div class="box-title">Notes / Remarks</div><?php if (! empty($h['notes'])): ?><div><?= esc($h['notes']) ?></div><?php endif ?><?php if (! empty($h['remarks'])): ?><div><?= nl2br(esc($h['remarks'])) ?></div><?php endif ?>
                    </div><?php endif ?></div>
            <?php if ($showAmounts): ?><table class="totals">
                    <tr>
                        <td>Subtotal</td>
                        <td class="text-end"><?= esc($money($subtotal)) ?></td>
                    </tr>
                    ...
                    <?php endif ?>
                    <?php if ($freight != 0.0): ?><tr>
                            <td>Freight</td>
                            <td class="text-end"><?= esc($money($freight)) ?></td>
                        </tr><?php endif ?>
                    <?php if ($other != 0.0): ?><tr>
                            <td>Other Amount</td>
                            <td class="text-end"><?= esc($money($other)) ?></td>
                        </tr><?php endif ?>
                    <?php if ($special != 0.0): ?><tr>
                            <td>Special Charge</td>
                            <td class="text-end"><?= esc($money($special)) ?></td>
                        </tr><?php endif ?>
                    <tr>
                        <td><?= $type === 'purchase-order' ? 'VAT' : 'Tax' ?></td>
                        <td class="text-end"><?= esc($money($tax)) ?></td>
                    </tr>
                    <?php if ($wht != 0.0): ?><tr>
                            <td>WHT</td>
                            <td class="text-end">(<?= esc($money($wht)) ?>)</td>
                        </tr><?php endif ?>
                    <tr>
                        <td>Grand Total</td>
                        <td class="text-end"><?= esc($money($total)) ?></td>
                    </tr>
                </table><?php endif ?>
        </div>
        <div class="signatures"><?php foreach ($signatures as $signature): ?><div class="signature">
                    <div class="line"><?= esc($signature) ?></div>
