            ...
            <div class="card-body">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-4">
                    <div>
                        ...
                        <p class="text-muted mb-0">Upload .xlsx, .csv, atau .tsv. File akan divalidasi terhadap dokumen sumber dan outstanding qty.</p>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="<?= site_url($templateUrl) ?>" class="btn btn-outline-primary"><i class="bx bx-download me-1"></i> Template</a>
                ...
                <form action="<?= site_url($importUrl) ?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" name="import_file" class="form-control" accept=".xlsx,.csv,.tsv" required>
                        <div class="form-text">Satu nomor <?= esc($typeLabel) ?> boleh punya banyak item. Warehouse/location berlaku per dokumen.</div>
                    </div>
                    <button type="submit" class="btn btn-primary"><i class="bx bx-search-alt me-1"></i> Preview Import</button>
                </form>
            </div>
        </div>
    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-nowrap mb-0">
                        ...
                        <?php foreach ($headers as $header): ?>
                            <tr><td><code><?= esc($header) ?></code></td></tr>
                        <?php endforeach ?>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
            ...
            <?php if (($preview['errors'] ?? []) === [] && ($previewToken ?? '') !== ''): ?>
                <form action="<?= site_url($commitUrl) ?>" method="post" class="mb-0">
                    ...
                    <button type="submit" class="btn btn-success"><i class="bx bx-check me-1"></i> Post Import</button>
                </form>
            <?php endif ?>
        </div>
            ...
            <table class="table table-sm table-bordered table-nowrap align-middle mb-0">
                ...
                <?php foreach (array_slice(($preview['valid_rows'] ?? []), 0, 100) as $row): ?>
                    <tr>
                        ...
                        <td class="text-end"><?= esc(number_format((float) ($row['qty'] ?? 0), 4)) ?></td>
                ...
                <?php if (($preview['valid_rows'] ?? []) === []): ?>
                    <tr><td colspan="8" class="text-center text-muted py-4">No valid rows.</td></tr>
                <?php endif ?>
                </tbody>
            </table>
        </div>
