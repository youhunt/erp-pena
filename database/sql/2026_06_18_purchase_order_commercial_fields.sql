-- PENA ERP manual SQL: Purchase Order commercial/date/remarks fields
...
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='discount_percent');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `discount_percent` DECIMAL(10,4) NOT NULL DEFAULT 0 AFTER `unit_price`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='freight_amount');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `freight_amount` DECIMAL(18,2) NOT NULL DEFAULT 0 AFTER `discount_amount`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='special_charge_amount');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `special_charge_amount` DECIMAL(18,2) NOT NULL DEFAULT 0 AFTER `freight_amount`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='vat_percent');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `vat_percent` DECIMAL(10,4) NOT NULL DEFAULT 0 AFTER `special_charge_amount`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='vat_amount');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `vat_amount` DECIMAL(18,2) NOT NULL DEFAULT 0 AFTER `vat_percent`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='wht_percent');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `wht_percent` DECIMAL(10,4) NOT NULL DEFAULT 0 AFTER `vat_amount`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='wht_amount');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `wht_amount` DECIMAL(18,2) NOT NULL DEFAULT 0 AFTER `wht_percent`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='delivery_date');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `delivery_date` DATE NULL DEFAULT NULL AFTER `line_status`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @col_exists := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db_name AND TABLE_NAME=@table_name AND COLUMN_NAME='arrive_date');
SET @sql := IF(@table_exists=1 AND @col_exists=0, 'ALTER TABLE `purchase_order_lines` ADD COLUMN `arrive_date` DATE NULL DEFAULT NULL AFTER `delivery_date`', 'SELECT 1'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
