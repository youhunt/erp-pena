...
### Journal Voucher yearly

```php
$number = $service->next('JV', null, [
    'company_id' => 1,
    'site_id' => 0,
    'prefix' => 'JV',
...
## 8. Catatan Penting Transaction Nesting

`DocumentNumberService::next()` sudah memakai database transaction internal. Jika dipanggil dari service yang juga membuka transaction, CodeIgniter akan menangani nested transaction sesuai konfigurasi connection.

Untuk flow enterprise yang kompleks, rekomendasi berikutnya adalah menambahkan method baru `nextWithinCurrentTransaction()` jika diperlukan setelah audit service transaksi existing.

## 9. Integrasi dengan Transaction Code dan Prefix Code
